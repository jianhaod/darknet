...
void run_dirdetector(int argc, char **argv) {
    ...
    char *datacfg = argv[2];
    char *cfgfile = argv[3];
    char *weightfile = (argc > 4) ? argv[4] : 0;
    char *dirpath = (argc > 5) ? argv[5]: 0;
	...
	while((entry = readdir(dp)) != NULL) {
        ...
        draw_dump_detections(im, l.w*l.h*l.n, thresh, boxes, probs, masks, names, alphabet, l.classes, filename);
        if(outfile){
